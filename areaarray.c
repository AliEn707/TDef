...
void initAreaArray(){
	...
	veci tmp1[12]={{-2,-1}, {-2,0}, {-2,1}, {-1,-2}, {-1,2}, {0,-2}, {0,2}, {1,-2}, {1,2}, {2,-1}, {2,0}, {2,1}};
	if((tmp[1]=malloc(sizeof(tmp1)))==0)
		perror("malloc tmp1 AreaArray");
	memcpy(tmp[1],tmp1,sizeof(tmp1));
	tmp_size[1]=12;

	veci tmp2[16]={{-3,-1}, {-3,0}, {-3,1}, {-2,-2}, {-2,2}, {-1,-3}, {-1,3}, {0,-3}, {0,3}, {1,-3}, {1,3}, {2,-2}, {2,2}, {3,-1}, {3,0}, {3,1}};
	if((tmp[2]=malloc(sizeof(tmp2)))==0)
		perror("malloc tmp2 AreaArray");
	memcpy(tmp[2],tmp2,sizeof(tmp2));
	tmp_size[2]=16;

	veci tmp3[32]={{-4,-2}, {-4,-1}, {-4,0}, {-4,1}, {-4,2}, {-3,-3}, {-3,-2}, {-3,2}, {-3,3}, {-2,-4}, {-2,-3}, {-2,3}, {-2,4}, {-1,-4}, {-1,4}, {0,-4}, {0,4}, {1,-4}, {1,4}, {2,-4}, {2,-3}, {2,3}, {2,4}, {3,-3}, {3,-2}, {3,2}, {3,3}, {4,-2}, {4,-1}, {4,0}, {4,1}, {4,2}};
	if((tmp[3]=malloc(sizeof(tmp3)))==0)
		perror("malloc tmp3 AreaArray");
	memcpy(tmp[3],tmp3,sizeof(tmp3));
	tmp_size[3]=32;

	veci tmp4[28]={{-5,-2}, {-5,-1}, {-5,0}, {-5,1}, {-5,2}, {-4,-3}, {-4,3}, {-3,-4}, {-3,4}, {-2,-5}, {-2,5}, {-1,-5}, {-1,5}, {0,-5}, {0,5}, {1,-5}, {1,5}, {2,-5}, {2,5}, {3,-4}, {3,4}, {4,-3}, {4,3}, {5,-2}, {5,-1}, {5,0}, {5,1}, {5,2}};
	if((tmp[4]=malloc(sizeof(tmp4)))==0)
		perror("malloc tmp4 AreaArray");
	memcpy(tmp[4],tmp4,sizeof(tmp4));
	tmp_size[4]=28;

	veci tmp5[40]={{-6,-2}, {-6,-1}, {-6,0}, {-6,1}, {-6,2}, {-5,-4}, {-5,-3}, {-5,3}, {-5,4}, {-4,-5}, {-4,-4}, {-4,4}, {-4,5}, {-3,-5}, {-3,5}, {-2,-6}, {-2,6}, {-1,-6}, {-1,6}, {0,-6}, {0,6}, {1,-6}, {1,6}, {2,-6}, {2,6}, {3,-5}, {3,5}, {4,-5}, {4,-4}, {4,4}, {4,5}, {5,-4}, {5,-3}, {5,3}, {5,4}, {6,-2}, {6,-1}, {6,0}, {6,1}, {6,2}};
	if((tmp[5]=malloc(sizeof(tmp5)))==0)
		perror("malloc tmp5 AreaArray");
	memcpy(tmp[5],tmp5,sizeof(tmp5));
	tmp_size[5]=40;

	veci tmp6[40]={{-7,-2}, {-7,-1}, {-7,0}, {-7,1}, {-7,2}, {-6,-4}, {-6,-3}, {-6,3}, {-6,4}, {-5,-5}, {-5,5}, {-4,-6}, {-4,6}, {-3,-6}, {-3,6}, {-2,-7}, {-2,7}, {-1,-7}, {-1,7}, {0,-7}, {0,7}, {1,-7}, {1,7}, {2,-7}, {2,7}, {3,-6}, {3,6}, {4,-6}, {4,6}, {5,-5}, {5,5}, {6,-4}, {6,-3}, {6,3}, {6,4}, {7,-2}, {7,-1}, {7,0}, {7,1}, {7,2}};
	if((tmp[6]=malloc(sizeof(tmp6)))==0)
		perror("malloc tmp6 AreaArray");
	memcpy(tmp[6],tmp6,sizeof(tmp6));
	tmp_size[6]=40;

	veci tmp7[48]={{-8,-2}, {-8,-1}, {-8,0}, {-8,1}, {-8,2}, {-7,-4}, {-7,-3}, {-7,3}, {-7,4}, {-6,-6}, {-6,-5}, {-6,5}, {-6,6}, {-5,-6}, {-5,6}, {-4,-7}, {-4,7}, {-3,-7}, {-3,7}, {-2,-8}, {-2,8}, {-1,-8}, {-1,8}, {0,-8}, {0,8}, {1,-8}, {1,8}, {2,-8}, {2,8}, {3,-7}, {3,7}, {4,-7}, {4,7}, {5,-6}, {5,6}, {6,-6}, {6,-5}, {6,5}, {6,6}, {7,-4}, {7,-3}, {7,3}, {7,4}, {8,-2}, {8,-1}, {8,0}, {8,1}, {8,2}};
	if((tmp[7]=malloc(sizeof(tmp7)))==0)
		perror("malloc tmp7 AreaArray");
	memcpy(tmp[7],tmp7,sizeof(tmp7));
	tmp_size[7]=48;

	veci tmp8[68]={{-9,-3}, {-9,-2}, {-9,-1}, {-9,0}, {-9,1}, {-9,2}, {-9,3}, {-8,-5}, {-8,-4}, {-8,-3}, {-8,3}, {-8,4}, {-8,5}, {-7,-6}, {-7,-5}, {-7,5}, {-7,6}, {-6,-7}, {-6,7}, {-5,-8}, {-5,-7}, {-5,7}, {-5,8}, {-4,-8}, {-4,8}, {-3,-9}, {-3,-8}, {-3,8}, {-3,9}, {-2,-9}, {-2,9}, {-1,-9}, {-1,9}, {0,-9}, {0,9}, {1,-9}, {1,9}, {2,-9}, {2,9}, {3,-9}, {3,-8}, {3,8}, {3,9}, {4,-8}, {4,8}, {5,-8}, {5,-7}, {5,7}, {5,8}, {6,-7}, {6,7}, {7,-6}, {7,-5}, {7,5}, {7,6}, {8,-5}, {8,-4}, {8,-3}, {8,3}, {8,4}, {8,5}, {9,-3}, {9,-2}, {9,-1}, {9,0}, {9,1}, {9,2}, {9,3}};
	if((tmp[8]=malloc(sizeof(tmp8)))==0)
		perror("malloc tmp8 AreaArray");
	memcpy(tmp[8],tmp8,sizeof(tmp8));
	tmp_size[8]=68;

	veci tmp9[56]={{-10,-3}, {-10,-2}, {-10,-1}, {-10,0}, {-10,1}, {-10,2}, {-10,3}, {-9,-5}, {-9,-4}, {-9,4}, {-9,5}, {-8,-6}, {-8,6}, {-7,-7}, {-7,7}, {-6,-8}, {-6,8}, {-5,-9}, {-5,9}, {-4,-9}, {-4,9}, {-3,-10}, {-3,10}, {-2,-10}, {-2,10}, {-1,-10}, {-1,10}, {0,-10}, {0,10}, {1,-10}, {1,10}, {2,-10}, {2,10}, {3,-10}, {3,10}, {4,-9}, {4,9}, {5,-9}, {5,9}, {6,-8}, {6,8}, {7,-7}, {7,7}, {8,-6}, {8,6}, {9,-5}, {9,-4}, {9,4}, {9,5}, {10,-3}, {10,-2}, {10,-1}, {10,0}, {10,1}, {10,2}, {10,3}};
